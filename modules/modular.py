...
def is_compatible(diffusion_pipeline: diffusers.DiffusionPipeline) -> bool:
    ...
    if not compatible:
        logger.log.debug(f'Modular: source={diffusion_pipeline.__class__.__name__} incompatible pipeline')
    return compatible
...
def convert_to_modular(diffusion_pipeline: diffusers.DiffusionPipeline) -> diffusers.ModularPipeline:
    ...
    try:
        ...
        t1 = time.time()
        logger.log.debug(f'Modular: source={diffusion_pipeline.__class__.__name__} target={modular_pipe.__class__.__name__} time={t1 - t0:.2f}')
        """
        for expected_input_param in modular_pipe.blocks.inputs:
            name = expected_input_param.name
            default = expected_input_param.default
            kwargs_type = expected_input_param.kwargs_type
            logger.log.trace(f'Modular input: name={name} type={kwargs_type} default={default}')
        """

    except Exception as e:
        logger.log.error(f'Modular: {e}')
        raise e
    return modular_pipe
...
def restore_standard(modular_pipe):
    if hasattr(modular_pipe, 'original_pipe'):
        logger.log.debug(f'Modular: source={modular_pipe.__class__.__name__} target={modular_pipe.original_pipe.__class__.__name__}')
