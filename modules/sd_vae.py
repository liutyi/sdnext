...
def get_vae_scale_factor(model=None):
    ...
    if model is None:
        ...
        # logger.log.warning(f'VAE: cls={model.__class__.__name__ if model else "None"} scale=unknown')
        vae_scale_factor = 8
    if hasattr(model, 'patch_size'):
        patch_size = model.patch_size
    if debug:
        logger.log.trace(f'VAE: cls={model.__class__.__name__ if model else "None"} scale={vae_scale_factor} patch={patch_size}')
...
def refresh_vae_list():
    ...
    logger.log.info(f'Available VAEs: path="{vae_path}" items={len(vae_dict)}')
    return vae_dict
...
def resolve_vae(checkpoint_file):
    ...
    if shared.opts.sd_vae == "Automatic": # 4th
        ...
        if vae_from_options is not None:
            return vae_from_options, 'settings'
        logger.log.warning(f"VAE not found: {shared.opts.sd_vae}")
    return None, None
...
def load_vae_diffusers(model_file, vae_file=None, vae_source="unknown-source"):
    ...
    if not os.path.exists(vae_file):
        logger.log.error(f'VAE not found: model{vae_file}')
        return None
    ...
    logger.log.info(f'Load module: type=VAE model="{vae_file}" source={vae_source} config={diffusers_load_config}')
    try:
        import diffusers
        if os.path.isfile(vae_file):
            if os.path.getsize(vae_file) > 1310944880: # 1.3GB
                ...
                if getattr(vae.config, 'scaling_factor', 0) == 0.18125 and shared.sd_model_type == 'sdxl':
                    ...
                    logger.log.debug('Setting model: component=VAE fix scaling factor')
        ...
        # logger.log.debug(f'Diffusers VAE config: {vae.config}')
        if shared.opts.diffusers_offload_mode == 'none':
            sd_models.move_model(vae, devices.device)
        return vae
    except Exception as e:
        logger.log.error(f"Load VAE failed: model={vae_file} {e}")
        if debug:
