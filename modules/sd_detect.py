...
def guess_by_size(fn, current_guess):
    ...
    if os.path.isfile(fn) and fn.endswith('.safetensors'):
        ...
        if (size > 0 and size < 128):
            logger.log.warning(f'Model size smaller than expected: file="{fn}" size={size} MB')
        elif (size >= 316 and size <= 324) or (size >= 156 and size <= 164): # 320 or 160
            logger.log.warning(f'Model detected as VAE model, but attempting to load as model: file="{fn}" size={size} MB')
            new_guess = 'VAE'
            ...
            new_guess = 'FLUX'
        if debug_load:
            logger.log.trace(f'Autodetect: method=size file="{fn}" size={size} previous="{current_guess}" current="{new_guess}"')
...
def guess_by_name(fn, current_guess):
    ...
    if 'instaflow' in fn.lower():
        ...
        if devices.dtype == torch.float16:
            logger.log.warning('Stable Cascade does not support Float16')
        ...
        if size > 11000 and size < 16000:
            logger.log.warning(f'Model detected as FLUX UNET model, but attempting to load a base model: file="{fn}" size={size} MB')
        new_guess = 'FLUX'
    ...
    if 'chronoedit' in fn.lower():
        ...
        new_guess = 'GLM-Image'
    if debug_load:
        logger.log.trace(f'Autodetect: method=name file="{fn}" previous="{current_guess}" current="{new_guess}"')
    return new_guess or current_guess
            ...
            for k, v in pipelines.items():
                if v is not None and v.__name__ == pipeline.__name__:
                    if is_quant:
                        k = f'{k} SDNQ'
                    if debug_load:
                        logger.log.trace(f'Autodetect: method=diffusers file="{fn}" previous="{current_guess}" current="{k}"')
                    return k, v
    return current_guess, None
...
def guess_variant(fn, current_guess):
    ...
    if 'inpaint' in fn.lower():
        ...
        if current_guess == 'Stable Diffusion':
            ...
            new_guess = 'Stable Diffusion XL Instruct'
    if debug_load:
        logger.log.trace(f'Autodetect: method=variant file="{fn}" previous="{current_guess}" current="{new_guess}"')
    return new_guess or current_guess
...
def detect_pipeline(f: str, op: str = 'model'):
    ...
    if guess == 'Autodetect':
        try:
            ...
            logger.log.info(f'Autodetect {op}: detect="{guess}" class={getattr(pipeline, "__name__", None)} file="{f}"')
            if debug_load is not None:
                ...
                if keys is not None and len(keys) > 0:
                    ...
                    t1 = time.time()
                    logger.log.debug(f'Autodetect: modules={modules} list={lst} time={t1-t0:.2f}')
        except Exception as e:
            logger.log.error(f'Autodetect {op}: file="{f}" {e}')
            if debug_load:
        ...
        try:
            pipeline = shared_items.get_pipelines().get(guess, None) if pipeline is None else pipeline
            logger.log.info(f'Load {op}: detect="{guess}" class={getattr(pipeline, "__name__", None)} file="{f}"')
        except Exception as e:
            logger.log.error(f'Load {op}: detect="{guess}" file="{f}" {e}')
