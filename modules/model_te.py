    ...
    if fn is not None and name.lower().endswith('gguf'):
        ...
        if t5.dtype != devices.dtype:
            try:
                ...
                logger.log.error(f"T5: Failed to cast text encoder to {devices.dtype}, set dtype to {t5.dtype}")
                raise
        del state_dict
        ...
        logger.log.debug(f'Load model: type=T5 repo={name}')
...
def set_t5(pipe, module, t5=None, cache_dir=None):
    ...
    try:
        ...
        logger.log.error(f'Load module: type={module} class="T5" file="{shared.opts.sd_text_encoder}" {e}')
        if debug:
            errors.display(e, 'TE:')
        t5 = None
    if t5 is None:
...
def set_clip(pipe):
    ...
    if 'vit-l' in shared.opts.sd_text_encoder.lower() and hasattr(shared.sd_model, 'text_encoder') and shared.sd_model.text_encoder.__class__.__name__ == 'CLIPTextModel':
        try:
            ...
            logger.log.error(f'Load module: type="text_encoder" class="ViT-L" file="{shared.opts.sd_text_encoder}" {e}')
            if debug:
                errors.display(e, 'TE:')
            te = None
        if te is not None:
            pipe.text_encoder = te
            logger.log.info(f'Load module: type="text_encoder" class="ViT-L" file="{shared.opts.sd_text_encoder}"')
    ...
    if 'vit-g' in shared.opts.sd_text_encoder.lower() and hasattr(shared.sd_model, 'text_encoder_2') and shared.sd_model.text_encoder_2.__class__.__name__ == 'CLIPTextModelWithProjection':
        try:
            ...
            logger.log.error(f'Load module: type module="text_encoder_2" class="ViT-G" file="{shared.opts.sd_text_encoder}" {e}')
            if debug:
                errors.display(e, 'TE:')
            te = None
        if te is not None:
            pipe.text_encoder_2 = te
            logger.log.info(f'Load module: type="text_encoder_2" class="ViT-G" file="{shared.opts.sd_text_encoder}"')
            import modules.prompt_parser_diffusers
            modules.prompt_parser_diffusers.cache.clear()
...
def refresh_te_list():
    ...
    for file in files_cache.list_files(shared.opts.te_dir, ext_filter=['.safetensors', '.gguf']):
        ...
        te_dict[name] = file
    logger.log.info(f'Available TEs: path="{shared.opts.te_dir}" items={len(te_dict)}')
