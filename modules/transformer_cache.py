...
def set_cache(faster_cache=None, pyramid_attention_broadcast=None):
    ...
    if (not hasattr(shared.sd_model.transformer, 'enable_cache')) or (not hasattr(shared.sd_model.transformer, 'disable_cache')):
        logger.log.debug(f'Transformer cache: cls={shared.sd_model.transformer.__class__.__name__} fc={faster_cache} pab={pyramid_attention_broadcast} not supported')
        return
    try:
        if faster_cache: # https://github.com/huggingface/diffusers/pull/10163
            ...
            logger.log.debug(f'Transformer cache: type={config.__class__.__name__}')
        ...
        logger.log.error(f'Transformer cache: {e}')
