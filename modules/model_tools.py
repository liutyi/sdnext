...
def get_modules(model: callable):
    ...
    for name, cls in params.items():
        logger.log.debug(f'Analyze: model={model} module={name} class={cls.__name__} loadable={getattr(cls, "from_pretrained", None)}')
    return params
...
def load_modules(repo_id: str, params: dict):
    ...
    for name, cls in params.items():
        ...
        if subfolder is None:
            continue
        logger.log.debug(f'Load: module={name} class={cls.__name__} repo={repo_id} location={subfolder}')
