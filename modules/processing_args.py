...
def task_specific_kwargs(p, model):
    ...
    if 'BlipDiffusionPipeline' in model_cls:
        if len(p.init_images) == 0:
            logger.log.error('BLiP diffusion requires init image')
            return task_args
        task_args = {
...
def set_pipeline_args(p, model, prompts:list, negative_prompts:list, prompts_2:list | None=None, negative_prompts_2:list | None=None, prompt_attention:str | None=None, desc:str | None='', **kwargs):
    ...
    if len(timesteps) > 2:
        if ('timesteps' in possible) and hasattr(model.scheduler, 'set_timesteps') and ("timesteps" in set(inspect.signature(model.scheduler.set_timesteps).parameters.keys())):
            ...
            logger.log.debug(f'Sampler: steps={len(p.timesteps)} timesteps={p.timesteps}')
            ...
            logger.log.debug(f'Sampler: steps={len(p.timesteps)} sigmas={p.timesteps}')
        else:
            logger.log.warning(f'Sampler: cls={model.scheduler.__class__.__name__} timesteps not supported')
    ...
    logger.log.info(f'{desc}: pipeline={model.__class__.__name__} task={task} batch={p.iteration + 1}/{p.n_iter}x{p.batch_size} set={clean}')

    if p.hdr_clamp or p.hdr_maximize or p.hdr_brightness != 0 or p.hdr_color != 0 or p.hdr_sharpen != 0:
        logger.log.debug(f'HDR: clamp={p.hdr_clamp} maximize={p.hdr_maximize} brightness={p.hdr_brightness} color={p.hdr_color} sharpen={p.hdr_sharpen} threshold={p.hdr_threshold} boundary={p.hdr_boundary} max={p.hdr_max_boundary} center={p.hdr_max_center}')
    if shared.cmd_opts.profile:
        t1 = time.time()
        logger.log.debug(f'Profile: pipeline args: {t1-t0:.2f}')
    if debug_enabled:
